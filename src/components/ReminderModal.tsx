        ...
        {!supported ? (
          ...
          <div className="flex flex-col gap-4">
            <label className="flex items-center justify-between gap-3">
              <div>
                <div className="text-base font-medium text-ink">เปิดการแจ้งเตือน</div>
            ...
            <label className="flex items-center justify-between gap-3">
              <span className="text-base font-medium text-ink">เวลา</span>
              <input
                type="time"
                value={reminder.time}
                onChange={(e) => onChange({ time: e.target.value })}
                className="rounded-xl border-0 bg-paper-2/50 px-3 py-2.5 text-base text-ink outline-none ring-1 ring-line focus:bg-elevated focus:ring-2 focus:ring-brand/40"
              />
            </label>

            {denied && (
              <p className="chip-danger rounded-xl px-3 py-2.5 text-sm">
            ...
            {reminder.enabled && Notification.permission === "granted" && (
              <button
                onClick={test}
                className="rounded-xl px-4 py-2.5 text-base font-medium text-brand ring-1 ring-brand/20 transition hover:bg-brand-soft"
              >
                ทดสอบแจ้งเตือน
              </button>
            )}
