  ...
  return (
    ...
    >
      <button onClick={onSelect} className="flex min-w-0 flex-1 items-start gap-3 px-2.5 py-2.5 text-left">
        <div
          className={`grid h-12 w-12 shrink-0 flex-col place-items-center rounded-xl leading-none ${
            active || isToday(day.date)
              ? "bg-brand text-on-brand"
              : "bg-paper-2 text-ink-soft"
          }`}
        >
          <span className="text-xs font-medium opacity-80">{formatWeekday(day.date)}</span>
          <span className="font-display text-lg font-semibold">{formatDayNum(day.date)}</span>
        </div>
        <div className="min-w-0 flex-1 pt-0.5">
          ...
          <p className="mt-0.5 line-clamp-1 text-sm text-ink-faint">{preview(day) || "—"}</p>
          {catSet.length > 0 && (
      ...
      <button
        ...
        className={`mr-1 mt-2.5 tap-target grid h-9 w-9 shrink-0 place-items-center rounded-lg transition ${
          day.pinned
            ? "text-amber-500"
      ...
      <div className="flex items-center justify-between px-5 pb-3 pt-5">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-display text-xl font-semibold tracking-tight text-ink">Daily Note</h2>
          </div>
          <p className="mt-1 pl-10 text-sm text-ink-faint">
        ...
        {onClose && (
          <button
            onClick={onClose}
            className="tap-target grid h-11 w-11 place-items-center rounded-xl text-ink-soft ring-1 ring-line transition hover:bg-elevated lg:hidden"
            aria-label="ปิดเมนู"
          >
            <X className="h-5 w-5" />
          </button>
      ...
      <nav className="flex-1 overflow-y-auto px-3 pb-6">
        {days.length === 0 ? (
          ...
          <>
            {pinned.length > 0 && (
              <div className="mb-2">
                <div className="mb-1 px-2 text-sm font-semibold uppercase tracking-wide text-ink-faint">
                  ปักหมุด
                </div>
